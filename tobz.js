...
MAU APIKEY NAUFALHOSTER.XYZ? MAMPIR AJA KE NAUFALHOSTER.XYZ

ELAINA BOT V3 MODIFIKASI DIKIT BY https://github.com/bangnopal
*/
...
/* 
MODIFIKASI DIKIT by BANGNOPAL
MODIFIKASI: MENAMBAHKAN APIKEY NAUFALHOSTER.XYZ
PENGEN APIKEY NAUFALHOSTER.XYZ NYA? MAMPIR AJA KE NAUFALHOSTER.XYZ
*/
let { 
    limitCount,
    memberLimit, 
...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '+923178022596@cus'
	...
	if (isGroupMsg && AntiStickerSpam && !isGroupAdmins && !isAdmin && !isOwner){
        ...
        case prefix+'blackpink':
            ...
            await tobz.sendFileFromUrl(from, `https://api.vhtear.com/blackpinkicon?apikey=${vhtearkey}&text=${blpk}`, 'blackpink.jpg', '', id)
            break
        case prefix+'thunder':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            ...
            /* W MODIFIKASI API URL NYA PAKE PUNYA W, SUPAYA USER YG BLOM PUNYA APIKEY VHTEAR BISA PAKE INI AJA */
            if (thndr.length > 20) return tobz.reply(from, '*Teks Terlalu Panjang!*\n_Maksimal 20 huruf!_', id)
            await tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/thunder?text=${thndr}&apikey=${turukey}`, 'thndr.jpg', '', id)
            break
/* W MODIFIKASI API URL NYA SUPAYA USER FREE BISA TETEP PAKE API INI AJA */
        case prefix+'marvel':
				if(isReg(obj)) return
				if(cekumur(cekage)) return
				if (isLimit(serial)) return
				if (args.length === 0) return tobz.reply(from, `Kirim perintah *${prefix} [ | Teks1 | teks2 ]*, contoh *${prefix} | marvel | anjai*`, id)
				argz = body.trim().split('|')
				if (argz.length >= 2) {
					tobz.reply(from, mess.wait, id)
					tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/marvel?apikey=${turukey}&text1=${argz[0]}&text2=${argz[1]}`, 'marvel.jpg', 'API Powered by naufalhoster.xyz', id)
				} else {
					await tobz.reply(from, `Format salah, contoh: *#marvel | marvel | anjai*`, id)
				break
				case prefix+'pentakill':
						if(isReg(obj)) return
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'Contoh: *#pentakill savage*', id)
						tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/pentakill?apikey=${turukey}&text=${body.slice(9)}`, 'pentakill.jpg', 'API Powered by naufalhoster.xyz', id)
				...
				case prefix+'pubg':
						...
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#pubg | pubg | epep', id)
						argz = body.trim().split(' | ')
						if (argz.length >= 2) {
							tobz.reply(from, mess.wait, id)
							tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/pubg?apikey=${turukey}&text1=${argz[0]}&text2=${argz[1]}`, 'pubg.jpg', 'API Powered by naufalhoster.xyz', id)
							await limitAdd(serial)
						} else {
							return tobz.reply(from, 'contoh: *#pubg | hello | world*', id)
						}
						break
				case prefix+'bokeh':
						if(isReg(obj)) return
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#bokeh hello world', id)
						tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/bokeh?apikey=${turukey}&text=${body.slice(6)}`, 'bokeh.jpg', 'API Powered by naufalhoster.xyz', id)
						await limitAdd(serial)
						break
        case prefix+'pornhub':
            if(isReg(obj)) return
            ...
            break
        case prefix+'googlesuggestion':
        		if(isReg(obj)) return
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#googlesuggestion | bot wa termuk | API naufalhoster.xyz | arab gila*', id)
						argz = body.trim().split(' | ')
						if (argz.length >= 2) {
							tobz.reply(from, mess.wait, id)
							tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/google_suggestion?apikey=${turukey}&text1=${argz[0]}&text2=${argz[1]}&text3=${argz[2]}`, 'google.jpg', 'API Powered by naufalhoster.xyz', id)
							await limitAdd(serial)
						} else {
							return tobz.reply(from, 'contoh: *#googlesuggestion | API naufalhoster.xyz | masker kecantikan | bedak pria*', id)
						}
						break
				case prefix+'fbgold':
						if(isReg(obj)) return
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#avenger | hello | world*', id)
						tobz.reply(from, mess.wait, id)
						tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/fb_golden_play_button?apikey=${turukey}&text=${body.slice(8)}`, 'facebook.jpg', 'API Powered by naufalhoster.xyz', id)
						await limitAdd(serial)
						break
				case prefix+'football':
						...
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#football | sinchan | 17*', id)
						argz = body.trim().split(' | ')
						if (argz.length >= 2) {
							tobz.reply(from, mess.wait, id)
							tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/football?apikey=${turukey}&text1=${argz[0]}&text2=${argz[1]}`, 'football.jpg', 'API Powered by naufalhoster.xyz', id)
							await limitAdd(serial)
						} else {
							return tobz.reply(from, 'contoh: *#avenger | sinchan | 17*', id)
						}
						break
				case prefix+'spiderman':
						...
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#spiderman | hello | world*', id)
						argz = body.trim().split(' | ')
						if (argz.length >= 2) {
							tobz.reply(from, mess.wait, id)
							tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/spiderman?apikey=${turukey}&text1=${argz[0]}&text2=${argz[1]}`, 'avenger.jpg', 'API Powered by naufalhoster.xyz', id)
							await limitAdd(serial)
						} else {
							return tobz.reply(from, 'contoh: *#spiderman | hello | world*', id)
						}
						break
        case prefix+'battlefield':
        		if(isReg(obj)) return
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#battlefield | API naufalhoster.xyz | fitur keren *', id)
						argz = body.trim().split(' | ')
						if (argz.length >= 2) {
							tobz.reply(from, mess.wait, id)
							tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/battlefield?apikey=${turukey}&text1=${argz[0]}&text2=${argz[1]}`, 'battlefield.jpg', 'API Powered by naufalhoster.xyz', id)
							await limitAdd(serial)
						} else {
							return tobz.reply(from, 'contoh: *#battlefield | hello | world*', id)
						}
						break
        case prefix+'avenger':
        		if(isReg(obj)) return
						...
						if (argz.length >= 2) {
							tobz.reply(from, mess.wait, id)
							tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/avenger?apikey=${turukey}&text1=${argz[0]}&text2=${argz[1]}`, 'avenger.jpg', 'API Powered by naufalhoster.xyz', id)
							await limitAdd(serial)
						} else {
							return tobz.reply(from, 'contoh: *#avenger | hello | world*', id)
						}
						break
        case prefix+'glitch':
            if(isReg(obj)) return
            ...
            if (argz.length >= 2) {
                ...
                if (glitch2.length > 20) return tobz.reply(from, '*Teks2 Terlalu Panjang!*\n_Maksimal 20 huruf!_', id)
                /* W MODIFIKASI API URL NYA PAKE PUNYA W SUPAYA USER YG BLOM PUNYA APIKEY TEAR BISA PAKE INI AJA */
                tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/glitch?text1=${glitch1}&text2=${glitch2}&apikey=${turukey}`)
                await limitAdd(serial)
            } else {
...
${desc}`)
        ...
        case prefix+'randomhentai':
            ...
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            /* W MODIFIKASI API URL NYA SUPAYA USER FREE BISA TETEP PAKE API INI AJA */
            tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/anime/randomhentai?apikey=${turukey}`, `RandomHentai.jpg`, 'Random Hentai!', id)
            await limitAdd(serial)
            break
        case prefix+'randomnsfwneko':
            ...
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            /* W MODIFIKASI API URL NYA SUPAYA USER FREE BISA TETEP PAKE API INI AJA */
            tobz.sendFileFromUrlromUrl(from, `https://naufalhoster.xyz/anime/randomnsfw?apikey=${turukey}`, `NsfwNeko.jpg`, 'NsfwNeko!', id)
            await limitAdd(serial)
            break
        ...
        case prefix+'randomblowjob':
            ...
            if (!isNsfw) return tobz.reply(from, 'command/Perintah NSFW belum di aktifkan di group ini!', id)
            await limitAdd(serial)
            tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/anime/randomhentai?apikey=${turukey}`, `RandoBlow${ext}`, 'Random Blowjob!', id)
            break
...
🌐️ *URL*: ${url}`
        ...
        case prefix+'ssphone':
            ...
            tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/tools/ssweb?url=${ssphone}&apikey=${vhtearkey}`, 'ssphone.jpg', '', id)
            await limitAdd(serial)
            break
        case prefix+'sspc':
            if(isReg(obj)) return
            ...
            /* W MODIFIKASI API URL NYA SUPAYA USER FREE BISA TETEP PAKE API INI AJA */
            tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/tools/ssweb?url=${sspc}&apikey=${vhtearkey}`, 'sspc.jpg', '', id)
            await limitAdd(serial)
            break
	case prefix+'bitly':
            if(isReg(obj)) return
        ...
        case prefix+'wiki':
            ...
            /* W MODIFIKASI API URL NYA SUPAYA USER FREE BISA TETEP PAKE API INI AJA */
            const wiki = await axios.get(`https://naufalhoster.xyz/tools/wiki?query=${queryz_}&apikey=${turukey}`)
            if (wiki.data.message) {
                tobz.reply(from, wiki.data.message, id)
            } else {
                tobz.reply(from, wiki.data.result.artikel, id)
                await limitAdd(serial)
            }
        case prefix+'kbbi':
            if(isReg(obj)) return
...
*Text : ${swrt}*`
         ...
         case prefix+'einstein':
         		if(isReg(obj)) return
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#einstein 2x2=4jt*', id)
						tobz.reply(from, mess.wait, id)
						tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/einstein?apikey=${turukey}&text=${body.slice(10)}`, 'einstein.jpg', 'API Powered by naufalhoster.xyz', id)
						await limitAdd(serial)
						break
         case prefix+'tahta':
             if(isReg(obj)) return
             ...
             /* W MODIFIKASI API URL NYA SUPAYA USER FREE BISA TETEP PAKE API INI AJA */
             await tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/harta_tahta?text=${jreng}&apikey=${turukey}`,`${jreng}.jpg`,`Harta Tahta ${jreng}`, id)        
             await limitAdd(serial)
             break
        case prefix+'rainbow':
        		if(isReg(obj)) return
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, '*#rainbow aku cinta dia*', id)
						tobz.reply(from, mess.wait, id)
						tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/rainbow?apikey=${turukey}&text=${body.slice(9)}`, 'rainbow.jpg', 'API Powered by naufalhoster.xyz', id)
						await limitAdd(serial)
						break
				case prefix+'naruto':
						if(isReg(obj)) return
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#naruto hello world*', id)
						tobz.reply(from, mess.wait, id)
						tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/naruto?apikey=${turukey}&text=${body.slice(8)}`, 'naruto.jpg', 'API Powered by naufalhoster.xyz', id)
						await limitAdd(serial)
						break
				case prefix+'crossfire':
						...
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#crossfire hello world*', id)
						tobz.reply(from, mess.wait, id)
						tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/crossfire?apikey=${turukey}&text=${body.slice(11)}`, 'crossfire.jpg', 'API Powered by naufalhoster.xyz', id)
						await limitAdd(serial)
						break
				case prefix+'warface':
						if(isReg(obj)) return
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#warface hello world*', id)
						tobz.reply(from, mess.wait, id)
						tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/warface?apikey=${turukey}&text=${body.slice(9)}`, 'warface.jpg', 'API Powered by naufalhoster.xyz', id)
						await limitAdd(serial)
						break
				case prefix+'csgo':
						if(isReg(obj)) return
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#csgo hello world', id)
						tobz.reply(from, mess.wait, id)
						tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/csgo?apikey=${turukey}&text=${body.slice(6)}`, 'csgo.jpg', 'API Powered by naufalhoster.xyz', id)
						await limitAdd(serial)
						break
				case prefix+'harrypotter':
						...
						if(cekumur(cekage)) return
						if (isLimit(serial)) return
						if (args.length === 0) return tobz.reply(from, 'contoh: *#harrypotter hello world', id)
						tobz.reply(from, mess.wait, id)
						tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/textmaker/harrypotter?apikey=${turukey}&text=${body.slice(13)}`, 'harrypotter.jpg', 'API Powered by naufalhoster.xyz', id)
						await limitAdd(serial)
						break
...
➸ *Total Rekaman:* ${recording}`
        ...
        case prefix+'tiktok':
            ...
            tobz.reply(from, mess.wait, id)
            /* W MODIFIKASI API URL NYA SUPAYA USER FREE BISA TETEP PAKE API INI AJA */
            const tiktokdlnaufalhoster = await axios.get(`https://naufalhoster.xyz/dl/tiktok?url=${body.slice(8)}&apikey=${turukey}`).then(result => {
            	tobz.sendFileFromUrl(from, result.data.result.thumbnail, 'tiktok.jpg', `Video ditemukan gan\n\nNama: ${result.data.result.name}\nCaption: ${result.data.result.caption}\nJumlah tayang: ${result.data.result.viewsCount}\n\n*Mohon tunggu proses pengiriman video gan*`, id)
            	tobz.sendFileFromUrl(from, result.data.result.url_video, 'tiktok.mp4', 'nih gan videonya', id)
            	await limitAdd(serial)
            }).catch(err => {
            	return tobz.reply(from, '❌ error gan, url yang anda berikan tidak valid', id)
            })
            break
        case prefix+'smule':
            if(isReg(obj)) return
            ...
            tobz.reply(from, mess.wait, id)
            const smuledl = await axios.get(`https://naufalhoster.xyz/dl/smule?apikey=${turukey}&url=${body.slice(7)}`).then(result => {
            	tobz.sendFileFromUrl(from, result.data.result.thumbnail, 'smule.jpg', `VIDEO & AUDIO DITEMUKAN\n\nJudul: ${result.data.result.title}\nCaption: ${result.data.result.caption}\n\n*Mohon tunggu proses pengiriman audio dan video nya gan*`, id)
            	tobz.sendFileFromUrl(from, result.data.result.audio, '', '', id)
            	tobz.sendFileFromUrl(from, result.data.result.video, 'smule.mp4', 'nih gan videonya', id)
            }).catch(err => {
            	return tobz.reply(from, '❌ error gan, link yang kamu berikan tidak valid')
            })
            break
        ...
        case prefix+'checkip':
            ...
            console.log(cekip)
            try {
             const cekdbip = await axios.get(`https://naufalhoster.xyz/tools/iplookup?apikey=${turukey}&ip=${body.slice(9)}`)
             if (cekdbip.data.code == "404") {
              return tobz.reply(from, cekdbip.data.message, id)
             } else {
              const resolt = cekdbip.data.result[0]
              tobz.sendFileFromUrl(from, resolt.flag.png, 'ip.png', `CEK IP API BY NAUFALHOSTER.XYZ\n\n➸ IP: ${resolt.ip_address}\n➸ Kota: ${resolt.city}\n➸ Benua: ${resolt.continent}\n➸ Negara: ${resolt.country}\n➸ Kode Negara: ${resolt.country_code}\n➸ Longtitude: ${resolt.longtitude}\n➸ Latitude: ${resolt.latitude}\n➸ Zona waktu: ${resolt.timezone.name}\n➸ Mata uang: ${resolt.currency.currency_name}\n➸ ISP: ${resolt.connection.isp_name}\n➸ Nama organisasi: ${resolt.connection.organization_name}\n➸ Koneksi type: ${resolt.connection.connection_type}`, id)
             }
            } catch (err) {
             return tobz.reply(from, 'IP tidak ditemukan', id)
            }
          break
        /*case prefix+'nhentai':
        case prefix+'nh':
        ...
        case prefix+'nulis':
            ...
            tobz.sendFileFromUrl(from, `https://naufalhoster.xyz/tools/textmaker?apikey=${turukey}&text=${ngettik}`, 'nulis.jpg', '', id)
            await limitAdd(serial)
            break
        case prefix+'inu':
            if(isReg(obj)) return
